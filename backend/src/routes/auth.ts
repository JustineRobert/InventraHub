...
router.post('/register', async (req, res, next) => {
  try {
    ...
    return res.status(201).json({ id: user.id, email: user.email, role: user.role, businessId: user.businessId });
  } catch (error) {
    next(error);
  }
});
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    res.cookie('inventrahub_token', token, {
      httpOnly: true,
    ...
    return res.json({ user: { id: user.id, email: user.email, role: user.role, businessId: user.businessId } });
  } catch (error) {
    next(error);
  }
});
...
router.post('/logout', authGuard, async (_req, res) => {
  res.clearCookie('inventrahub_token', { path: '/' });
